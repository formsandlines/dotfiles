...
(defconst ph/meow-common
  (list
   ...
   '("C-S-<up>" . enlarge-window) ;; C-S-i must be separated from TAB first
...
** Repeat like Vi
? is this useful -> does not work well with meow
#+begin_src elisp
...
;; Compiler Warnings:
;; ⛔ Warning (comp): dot-mode.el:454:20: Warning: Use keywords rather than deprecated positional arguments to `define-minor-mode'
;; ⛔ Warning (comp): dot-mode.el:509:27: Warning: defcustom for `global-dot-mode' fails to specify containing group
...
(use-package popwin
  ...
  ;; Issue: calling popup help window replaces popup with a regular window.
  ;; Thanks to user 'rakete' for this workaround:
  ;; - https://github.com/emacsorphanage/popwin/issues/131#issuecomment-239221901
  ...
  (keymap-global-set "C-`" #'ph/popwin-toggle-popup))
#+end_src
*** Popper
#+begin_src elisp
(use-package popper
...
(keymap-global-set "C-}" #'newline-empty-below)
(keymap-global-set "C-{" #'newline-empty-above)
...
;; (keymap-global-set "C-S-j" #'join-with-next-line) ; ? or C-c j
...
To consider:
- [ ] remove dot-mode, since it doesn’t work well with meow
